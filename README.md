...
For Claude Code, Codex, and GitHub Copilot, pass a selected candidate ID to the
...
They default to `~/.claude/skills`, `~/.codex/skills`, and `~/.copilot/skills`
respectively. These hosts discover skills at process/session boundaries, so
their JSON result sets `reload_required: true`; begin a new session before
relying on the skill. GitHub Copilot CLI can instead refresh an existing
interactive session with `/skills reload`. Use `SKILLET_CLAUDE_SKILLS_DIR`,
`SKILLET_CODEX_SKILLS_DIR`, `SKILLET_COPILOT_SKILLS_DIR`, and `SKILLET_TOKEN`
for overrides.
...
- **Client integrations:** native host materialisation, automatic resource-link downloads, deeper harness lifecycle integration, and lockfile maintenance by capable MCP hosts. The initial Pi, Claude Code, Codex, and GitHub Copilot adapters are intentionally thin and explicit.
