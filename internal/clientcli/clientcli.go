// Package clientcli implements the harness-neutral Skillet client command.
package clientcli
...
// Main runs the generic Skillet client command.
...
func materialize(args []string) {
	...
	_ = f.Parse(args)
	if (*candidate == "" && *version == "" && *versionRange == "") || (*candidate != "" && (*version != "" || *versionRange != "")) || (*version != "" && *versionRange != "") || ((*version != "" || *versionRange != "") && *skillID == "") || *destination == "" {
		fail("exactly one of -candidate, -version, or -range plus -destination is required")
	}
	...
	printJSON(map[string]any{"skill": out.Skill, "entrypoint": path, "lifecycle": out.Lifecycle, "activation": "materialized"})
}
...
func lifecycle(args []string) {
	...
	source := f.String("source", "", "optional integration identifier")
...
func usage() {
	fmt.Fprintln(os.Stderr, "usage: skillet-client search|materialize|lifecycle|feedback|feedback-list ...")
	os.Exit(2)
}
