...
func (distributionBrowserValidator) Authenticate(string) (authn.Identity, error) {
	return authn.Identity{
		Subject: "distribution-browser", OrganizationID: "demo",
...
func TestDistributionBrowserAcceptance(t *testing.T) {
	...
	if _, err := db.ExecContext(ctx, `UPDATE repositories SET url=? WHERE id=?`, "https://github.com/example/central-skills.git", "demo/central"); err != nil { t.Fatal(err) }
	if _, err := db.ExecContext(ctx, `UPDATE repositories SET url=? WHERE id=?`, "https://github.com/example/private-skills.git", "demo/private"); err != nil { t.Fatal(err) }
	...
	if err := app.ConfigureCollections([]composition.Collection{{
		ID: "review-kit", Name: "Review kit", Description: "root plus declared dependency",
	...
	t.Run("authorized_manifest_is_stable_pinned_and_scope_filtered", func(t *testing.T) {
		...
		if status != http.StatusOK { t.Fatalf("status=%d body=%s", status, first) }
		...
		if err := json.Unmarshal(first, &manifest); err != nil { t.Fatal(err) }
		...
		for _, plugin := range manifest.Plugins {
			...
			if strings.HasPrefix(plugin.Description, "DEPRECATED: ") { deprecatedVisible = true }
		}
		for _, want := range []string{"review-base", "review-root", "old-review"} {
			if !paths[want] { t.Fatalf("missing expected skill path %q: %+v", want, paths) }
		}
		if !deprecatedVisible { t.Fatal("deprecated capability was not explicit") }
		for _, forbidden := range []string{"withdrawn-review", "withdrawn fixture", "private-review", "restricted fixture"} {
			if strings.Contains(string(first), forbidden) { t.Fatalf("unauthorized/yanked metadata leaked: %q", forbidden) }
		}
	})

	t.Run("browser_view_has_provenance_commands_and_no_secret", func(t *testing.T) {
		status, _, body := get("/ui/distribution")
		if status != http.StatusOK { t.Fatalf("status=%d body=%s", status, body) }
		text := string(body)
		for _, want := range []string{"Claude Code marketplace export", "Download marketplace.json", "claude plugin marketplace add ./skillet-marketplace", "Manifest SHA-256", "review-root", "2.0.0", "review-kit"} {
			if !strings.Contains(text, want) { t.Fatalf("distribution UI missing %q in %s", want, text) }
		}
		for _, forbidden := range []string{"distribution-token", "withdrawn-review", "private-review"} {
			if strings.Contains(text, forbidden) { t.Fatalf("distribution UI leaked %q", forbidden) }
		}
	})
	...
	t.Run("collection_exports_exact_dependency_closure", func(t *testing.T) {
		...
		for _, want := range []string{"review-base", "review-root"} {
			if !strings.Contains(text, want) { t.Fatalf("collection missing %q: %s", want, text) }
		}
		for _, forbidden := range []string{"old-review", "withdrawn-review", "private-review"} {
			if strings.Contains(text, forbidden) { t.Fatalf("collection leaked unrelated capability %q: %s", forbidden, text) }
		}
	})

	t.Run("repository_scope_can_be_selected_without_cross_scope_leakage", func(t *testing.T) {
		status, _, body := get("/v1/distribution/claude-code/marketplace.json?namespace=team&repository=private")
		if status != http.StatusOK { t.Fatalf("status=%d body=%s", status, body) }
		if !strings.Contains(string(body), "private-review") || strings.Contains(string(body), "review-root") {
			t.Fatalf("scope projection incorrect: %s", body)
		}
	})
	...
	t.Run("m2_denial_filters_entries_and_collection_fails_without_dependency_leak", func(t *testing.T) {
		...
		if status != http.StatusOK { t.Fatalf("status=%d body=%s", status, body) }
		if !strings.Contains(string(body), "review-root") { t.Fatalf("authorized item missing: %s", body) }
		for _, forbidden := range []string{"review-base", "old-review", "private-review", "withdrawn-review"} {
			if strings.Contains(string(body), forbidden) { t.Fatalf("denied identity leaked: %q in %s", forbidden, body) }
		}

		status, _, body = get("/v1/distribution/claude-code/marketplace.json?collection=review-kit")
		if status != http.StatusBadRequest { t.Fatalf("status=%d body=%s", status, body) }
		if strings.Contains(string(body), "review-base") { t.Fatalf("denied dependency leaked in refusal: %s", body) }
	})
}

// Keep the os import used directly in this file so this acceptance fixture also
// fails clearly if repository setup disappears during future refactors.
