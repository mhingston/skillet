...
Skillet vNext treats correctness evidence as part of the change, not as a follow-up activity. The deterministic verification gate is the minimum proof required for code changes that can affect catalogue admission, routing, materialisation, restoration, provenance, or client-visible MCP behaviour.
...
4. the explicit offline admission/search/materialisation E2E journey in `internal/e2e`.
5. the versioned retrieval evaluation with the checked-in protected baseline.

The gate writes machine-readable evidence to `artifacts/verification/`:

- `retrieval.json` is the full retrieval evaluator report;
...
- **Negative/security tests** prove fail-closed behaviour such as digest mismatch, provenance mismatch, authentication boundaries, package safety, and bounded telemetry/feedback inputs.
...
`evals/retrieval.yaml` remains the source of acceptance thresholds. `evals/baselines/retrieval-v1.json` is the deterministic regression baseline. The protected metrics are:
...
The existing `max_regression` fixture value defines the permitted movement from the checked-in baseline. CI supplies the baseline on every run; baseline comparison is therefore mandatory rather than an optional local mode.

Do not add a threshold for a future knowledge/RAG subsystem until that subsystem has a representative fixture set and a metric whose failure meaning is understood.

### Updating a baseline

A baseline change is a deliberate review event, not routine snapshot churn. Update it only when the retrieval fixture, intended ranking semantics, or an accepted algorithmic trade-off changes. In the PR:
...
5. run `go run ./cmd/skillet-verify` and attach or quote the metric rows from `verification.json`.
...
Prefer focused tests for individual failure modes and keep the E2E journey centred on cross-component wiring and public behaviour.
...
- Result: PASS
- Protected metrics: <copy the four metric rows or concise observed/required values>
